...
% now lets see anti diffusion




ni = - 0.001; %diffusity or viscosity (instead of a2)
            %if this is negative, it means we are trying to put the paint
            %back into the cap

L = 10;
N = 400;
T = 0.1; % the more we wait, the more forward it goes and more it spreads

alpha = 100; % for the new function

 u = 1;
% 
 a1 = -0; %put velocity to zero bcz no translation
 
 
  % a1*T it says how much and in which direction we should go to take 
...
% C0 is a function of space, it is the blue line in print in time 0, then
% if we want it at other times, we just move the new function which we created
% with fourier series with T
...
alphak = a2* ( j   * omegak).^2 + ( j * a1 * omegak);


 %a2 and omega k are positive, j is imaginary unit
